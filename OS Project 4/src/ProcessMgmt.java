...
public class ProcessMgmt 
{
	public static void main(String args[]) 	
	{
		...
		LinkedBlockingQueue<PCB> QWaiting = new LinkedBlockingQueue<PCB>();
			...
			if(!QReady.isEmpty())
			{
				...
				if(memOpen.findOpenMem(PCB_Running, QMemOpen, QMemUsed))
				{
					...
					if (event__X == COMPLETED)
					{
						...
						AllocateMem(PCB_Running, QMemOpen, QMemUsed);
					}
					else
					{
				...
				{
					PCB_Switch = QWaiting.remove();
					PCB_Switch.set_state("Ready");
					System.out.println("********Process " + PCB_Switch.get_ID() + " finished I/O. Returned to Ready Que********");
					QReady.add(PCB_Switch);
				}
				//Display QReady
				System.out.println("\n*****\t\t\t\tReady Queue\t\t\t*****");   
				for (PCB pcbLoop : QReady)
